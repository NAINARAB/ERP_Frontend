...
const CreditNotePrintModal = ({ open, onClose, creditNote, loadingOn, loadingOff }) => {
    ...
    useEffect(() => {
        ...
        const noteId = creditNote.CR_Id || creditNote.DB_Id;

        if (noteId) {
            if (loadingOn) loadingOn();
            fetchLink({
                address: `creditNote/editDetails?CR_Id=${noteId}`
            }).then(data => {
                if (data?.success && data?.data) {
                    setFullDetails({
                        ...creditNote,
                        ...data.data,
                        Products_List: data.data.Products_List || creditNote.Products_List || creditNote.Product_Array || [],
                        Expence_Array: data.data.Expence_Array || creditNote.Expence_Array || [],
                    });
                } else {
                    setFullDetails(creditNote);
                }
            }).catch(e => {
                console.error(e);
                setFullDetails(creditNote);
    ...
    const activeData = fullDetails || creditNote || {};
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: `CreditNote_${activeData.CR_Inv_No || activeData.DB_Inv_No || 'Print'}`,
        pageStyle: `
            @page {
    ...
    const isInclusive = gstInclusive === 1;
    const taxType = isNotTaxableVal ? 'zerotax' : isInclusive ? 'remove' : 'add';
    ...
    products.forEach(prod => {
        ...
        const rawTaxRate = Number(prod.Tax_Rate || (Number(prod.Cgst || 0) + Number(prod.Sgst || 0)) || prod.Igst || 0);
        ...
        if (isNotTaxableVal) {
            taxable = Number(prod.Taxable_Amount || prod.Amount || (qty * rate));
            cgstAmo = 0;
            sgstAmo = 0;
            igstAmo = 0;
            cgstPer = 0;
            ...
            cgstPer = Number(prod.Cgst || (rawTaxRate ? rawTaxRate / 2 : 0));
            sgstPer = Number(prod.Sgst || (rawTaxRate ? rawTaxRate / 2 : 0));
            igstPer = Number(prod.Igst || rawTaxRate || 0);

            if (prod.Taxable_Amount !== undefined && prod.Taxable_Amount !== null && (prod.Cgst_Amo !== undefined || prod.Igst_Amo !== undefined)) {
                taxable = Number(prod.Taxable_Amount);
                cgstAmo = Number(prod.Cgst_Amo || 0);
                sgstAmo = Number(prod.Sgst_Amo || 0);
                igstAmo = Number(prod.Igst_Amo || 0);
            } else {
                const totalItemAmount = Number(prod.Amount || (qty * rate));
                const gstCalc = calculateGSTDetails(totalItemAmount, rawTaxRate, taxType);
                taxable = Number(gstCalc.without_tax);
                cgstAmo = isIGST ? 0 : Number(gstCalc.cgst_amount);
                sgstAmo = isIGST ? 0 : Number(gstCalc.sgst_amount);
                igstAmo = isIGST ? Number(gstCalc.igst_amount) : 0;
            }
        }
    ...
    const documentTitleHeader = isTaxableBill ? 'Tax Invoice' : 'Bill of Supply';
    const noteNumber = activeData.CR_Inv_No || activeData.DB_Inv_No || '';
    const noteDate = activeData.CR_Date || activeData.DB_Date;
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle sx={{ m: 0, p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>Credit Note Print Preview ({documentTitleHeader})</span>
                <div>
                            ...
                            <div style={{ width: '45%', display: 'flex', flexDirection: 'column' }}>
                                <div style={{ display: 'flex', borderBottom: '1px solid #000' }}>
                                    <div style={{ width: '50%', borderRight: '1px solid #000', padding: '3px 5px', minHeight: '34px' }}>
                                        <div style={{ fontSize: '11px' }}>Credit Note No.</div>
                                        <div style={{ fontWeight: 'bold' }}>{noteNumber}</div>
                                    </div>
                                    <div style={{ width: '50%', padding: '3px 5px' }}>
                                        <div style={{ fontSize: '11px' }}>Dated</div>
                                        <div style={{ fontWeight: 'bold' }}>{formatDateStandard(noteDate)}</div>
                                    </div>
                                </div>
            ...
            <DialogActions sx={{ p: 1.5, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div className="form-check form-switch ms-2">
                    <input
                        ...
                        type="checkbox"
                        id="creditNotePageSizeSwitch"
                        checked={pageSize === 'A4'}
                        onChange={() => setPageSize(prev => prev === 'A5' ? 'A4' : 'A5')}
                    />
                    <label className="form-check-label ms-1" htmlFor="creditNotePageSizeSwitch">
                        {pageSize} selected
                    </label>
                </div>
                <div>
...
export default CreditNotePrintModal;
