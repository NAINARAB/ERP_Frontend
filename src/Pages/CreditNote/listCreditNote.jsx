...
const CreditNoteList = ({ loadingOn, loadingOff, AddRights, EditRights, DeleteRights, pageID }) => {
    ...
    useEffect(() => {
        fetchLink({
            address: `creditNote/filterValues`
        }).then(data => {
            if (data.success) {
                setFiltersDropDown({
    ...
    useEffect(() => {
        ...
        fetchLink({
            address: `creditNote?Fromdate=${Fromdate}&Todate=${Todate}&Retailer_Id=${Retailer?.value || ''}&Created_by=${CreatedBy?.value || ''}&VoucherType=${VoucherType?.value || ''}&Cancel_status=${Cancel_status}`,
            loadingOn,
            loadingOff
        }).then(data => {
            if (data.success) {
    ...
    const totalValues = useMemo(() => {
        return creditNoteData.filter(inv => !isEqualNumber(inv.Cancel_status, 0)).reduce((acc, item) => {
                                ...
                                <ButtonActions
                                    buttonsData={[
                                        ...
                                        },
                                        {
                                            ...
                                            disabled: !DeleteRights || !canEditNow(row.CR_Date),
                                        },
