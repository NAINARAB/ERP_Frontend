...
import {
    debitNoteGeneralInfo, debitNoteDetailsInfo, debitNoteExpencesInfo,
    debitNoteStaffInfo, retailerOutstandingDetails, defaultStaffTypes
} from './variable';
import InvolvedStaffsDebitNote from "./manageInvolvedStaff";
import ManageDebitNoteGeneralInfo from "./manageGeneralInfo";
import DebitNoteTaxDetails from "./taxDetails";
import ExpencesOfDebitNote from "./manageExpences";
import AddProductFormDebitNote from "./addProducts";
...
const CreateDebitNote = ({ loadingOn, loadingOff }) => {
    ...
    const [invoiceInfo, setInvoiceInfo] = useState(debitNoteGeneralInfo);
    ...
    const isEdit = useMemo(() => isValidNumber(invoiceInfo?.DB_Id), [invoiceInfo?.DB_Id]);
        ...
        const fetchData = async () => {
            try {
                ...
                const [
                    ...
                    fetchLink({ address: `masters/defaultAccountMaster?Type=DEBIT_NOTE` }),
                    fetchLink({ address: `purchase/stockItemLedgerName?type=DEBIT_NOTE` }),
    ...
    useEffect(() => {
        if (isValidObject(editValues) && Array.isArray(editValues?.Product_Array)) {
            ...
            setInvoiceInfo(
                Object.fromEntries(
                    Object.entries(debitNoteGeneralInfo).map(([key, value]) => {
                        if (key === 'DB_Date' || key === 'Ref_Inv_Date') {
            ...
            setInvoiceExpences(
                toArray(Expence_Array).map(item => Object.fromEntries(
                    Object.entries(debitNoteExpencesInfo).map(([key, value]) => {
                        return [key, item[key] ?? value];
                    })
                ))
            );
            ...
            setStaffArray(() => {
                const stateOfStaff = toArray(Staffs_Array).map(item => Object.fromEntries(
                    Object.entries(debitNoteStaffInfo).map(([key, value]) => {
                        return [key, item[key] ?? value];
                    })
                ));
    ...
    const saveDebitNote = () => {
        if (isValidNumber(invoiceInfo?.DB_Id) && filterableText(invoiceInfo?.Alter_Reason).length === 0) {
        ...
        fetchLink({
            address: `debitNote`,
            method: checkIsNumber(invoiceInfo?.DB_Id) ? 'PUT' : 'POST',
    ...
    const fetchInvoiceProducts = () => {
        if (!stringCompare(invoiceInfo.Ref_Inv_Number, '')) {
            fetchLink({
                address: `purchase/purchaseOrder?Po_Inv_No=${invoiceInfo.Ref_Inv_Number}&Retailer_Id=${invoiceInfo.Retailer_Id}`,
        ...
        <>
            ...
            <Card>
                <div className='d-flex flex-wrap align-items-center border-bottom py-2 px-3'>
                    <span className="flex-grow-1 fa-16 fw-bold">Debit Note</span>
                    <span>
                        <Button type='button' onClick={() => {
                            if (window.history.length > 1) {
                                navigate(-1);
                            } else {
                                navigate('/erp/debitNote');
                            }
                        }}>Cancel</Button>
                        <Button onClick={saveDebitNote} variant="contained" disabled={invoiceProducts.length === 0}>submit</Button>
                    </span>
                </div>
                ...
                <CardContent>
                    ...
                    <FilterableTable
                        ...
                        columns={[
                            ...
                            createCol('Amount', 'number', 'Amount'),
                            {
                                ...
                                isCustomCell: true,
                                Cell: ({ row }) => {
                                    if (row?.Item_Id === 'TOTAL_ROW') return <></>;
                                    return (
                                        <>
                                            <IconButton
                                                size='small'
                    ...
                    <div className="row mt-3">
                        ...
                        <div className="col-md-5 p-2 bg-light border">
                            ...
                            <DebitNoteTaxDetails invoiceProducts={invoiceProducts} />
                        </div>
