...
const BranchwiseReport = ({ loadingOn, loadingOff }) => {
...
const BranchwiseSalesReport = ({ loadingOn, loadingOff, Fromdate, Todate }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetchLink({
          address: `sales/saleOrderReport?Fromdate=${Fromdate}&Todate=${Todate}`,
          ...
          const mergedData = rawData.map((branch) => {
            ...
            branch.Products?.forEach((p) => {
              if (!productMap[p.Product_Id]) {
                ...
                productMap[p.Product_Id].Sales_Quantity += p.Sales_Quantity;
              }
            });
            const products = Object.values(productMap).map((p) => ({
              ...p,
              Average_Daily_Sales: (p.Sales_Quantity / daysDiff).toFixed(2),
            }));
            return { ...branch, Products: products };
          });
  ...
  return (
    <FilterableTable
      ...
      isExpendable
      expandableComp={(props) => <ExpandableComp {...props} />}
      EnableSerialNumber
      filterOption
      initialPageCount={10}
    ...
    return (
      (appliedFilters.Product_Id === "" ||
        p.Product_Id?.toString()
          .toLowerCase()
          .includes(appliedFilters.Product_Id.toLowerCase())) &&
  ...
  return (
    <div className="p-2">
      ...
      <Dialog
        ...
        fullWidth
      >
        <DialogTitle>Filter Products in {row.BranchName}</DialogTitle>
      ...
      <FilterableTable
        ...
        CellSize="small"
        title={`Products in ${row.BranchName}`}
      />
    </div>
  );
};
