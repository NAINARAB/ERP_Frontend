...
const SalesDeliveryConvert = ({ loadingOn, loadingOff }) => {
    ...
    useEffect(() => {
        ...
        fetchLink({
            address: `sales/saleDelivery?Fromdate=${appliedFilters?.Fromdate}&Todate=${appliedFilters?.Todate}&Retailer_Id=${appliedFilters?.Retailer_Id}&Sales_Person_Id=${appliedFilters?.Sales_Person_Id}&Created_by=${appliedFilters?.Created_by}&Cancel_status=${appliedFilters?.Cancel_status}&Route_Id=${appliedFilters?.Route_Id}&Area_Id=${appliedFilters?.Area_Id}`,
        })
            .then((data) => {
                if (data.success) {
        ...
        appliedFilters?.Retailer_Id,
        appliedFilters?.Sales_Person_Id,
        appliedFilters?.Created_by,
        appliedFilters?.Cancel_status,
        appliedFilters?.Route_Id,
        appliedFilters?.Area_Id,
        reload
    ]);
    ...
    // ✅ Get only selectable rows (isConverted === 0)
    const getSelectableRows = () => {
        return saleOrders.filter(row => row?.isConverted === 0);
    };

    // ✅ Check if all selectable rows are selected
    const isAllSelectableRowsSelected = () => {
        const selectableRows = getSelectableRows();
        if (selectableRows.length === 0) return false;
        return selectableRows.every(row =>
            selectedRows.some(selectedRow => selectedRow.So_Id === row.So_Id)
        );
    };
    ...
    const handleSelectAll = () => {
        ...
        if (isAllSelectableRowsSelected()) {
            // Deselect all selectable rows
            setSelectedRows(selectedRows.filter(row => row?.isConverted !== 0));
        } else {
            // Select all selectable rows that aren't already selected
            const newSelectedRows = [...selectedRows];
            selectableRows.forEach(row => {
                if (!selectedRows.some(selected => selected.So_Id === row.So_Id)) {
            ...
            Cell: ({ row }) => {
                ...
                return (
                    <input
                        ...
                        style={{
                            cursor: row?.isConverted !== 0 ? "not-allowed" : "pointer",
                            transform: "scale(1.5)",
                            width: "14px",
                            height: "20px",
                        }}
    ...
    const selectableRowsCount = getSelectableRows().length;
    const allSelected = isAllSelectableRowsSelected();
                ...
                <div className="p-3 py-2 d-flex align-items-center justify-content-between">
                    ...
                    <div className="d-flex align-items-center gap-2">
                            ...
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        ...
                                        size="small"
                                        indeterminate={selectedRows.length > 0 && selectedRows.length < selectableRowsCount && !allSelected}
                                        disabled={isFiltersLoading}
                                    />
                                }
                                label={`Select All (${selectedRows.length}/${selectableRowsCount})`}
                                sx={{ mr: 2 }}
                            />
                        )}
                        ...
                        {selectedRows.length > 0 && screen && (
                            ...
                            >
                                Convert To Delivery ({selectedRows.length})
                            </Button>
                        )}
            ...
            >
                ...
                <DialogActions>
                    ...
                    <Button
                        ...
                        disabled={isFiltersLoading}
                    >
                        {isFiltersLoading ? "Searching..." : "Search"}
                    </Button>
                </DialogActions>
            </Dialog>
        </>
