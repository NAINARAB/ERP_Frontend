...
export default function DownloadSalesOrder() {
  ...
  const downloadPDF = useCallback(async () => {
    ...
    if (!printRef.current) {
      const fallbackEl = document.querySelector('[data-print-ref]') ||
                         document.querySelector('.sale-order-content');
      if (fallbackEl) {
        printRef.current = fallbackEl;
      } else {
        alert('Unable to generate PDF: Content not ready. Please try again.');
        return;
      }
    }
    ...
    try {
      // Mobile optimization: ensure content is fully rendered
      ...
      alert('Failed to generate PDF. Please try again or use the download button.');
    } finally {
  ...
  // AUTO DOWNLOAD — single consolidated effect. (The original page had three
  // separate effects all attempting this at once, which could race and fire
  // multiple downloads — consolidated here into one, and skipped entirely in
  // preview mode so embedding this page in the Preview popup never downloads
  // anything on its own.)
...
const safeProducts = products.map(p => {
 ...
 <div ref={printRef}  style={{
  ...
  margin: '0 auto',
        ...
        <h2 style={{ textAlign: 'center', margin: '0 0 8px', fontSize: isMobile ? '14px' : '18px' }}>
          SALE ORDER
        </h2>
        ...
        <div style={{ border: '1px solid #000', marginBottom: 10 }}>
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            ...
            <div style={{
              ...
              <div style={{ marginBottom: 6 }}>
                <div style={{ fontSize: isMobile ? '14px' : '18px', fontWeight: 'bold', marginBottom: 3 }}>
                  {companyInfoDetails?.Company_Name || inv.Branch_Name}
                </div>
                <div style={{ wordBreak: 'break-word', fontSize: isMobile ? '8px' : '10.5px' }}>{companyInfoDetails?.Company_Address}</div>
                <div style={{ fontSize: isMobile ? '8px' : '10.5px' }}>GSTIN/UIN: {companyInfo?.Gst_Number || companyInfoDetails?.Gst_Number || '-'}</div>
                <div style={{ fontSize: isMobile ? '8px' : '10.5px' }}>Region: {companyInfoDetails?.Region}, State: {companyInfoDetails?.State}</div>
                <div style={{ fontSize: isMobile ? '8px' : '10.5px' }}>Contact: {companyInfoDetails?.Telephone_Number}</div>
              </div>

              <div style={{ borderBottom: '1px solid #000', borderTop: '1px solid #000', paddingBottom: 6, marginBottom: 6, wordBreak: 'break-word', fontSize: isMobile ? '8px' : '10.5px' }}>
              ...
              <div style={{ wordBreak: 'break-word', fontSize: isMobile ? '8px' : '10.5px' }}>
            ...
            <div style={{ width: '50%', overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: isMobile ? '8px' : '10.5px' }}>
                <tbody>
                  {[
                    ...
                    <tr key={i}>
                      <td style={{ ...tdBase, borderRight: '1px solid #000', color: '#555', whiteSpace: 'nowrap', padding: '3px 5px' }}>
                        <strong>{l1}</strong><br />
                        <div style={{ fontWeight: 'bold', fontSize: isMobile ? '9px' : '11.5px' }}>{v1}</div>
                      </td>
                      <td style={{ ...tdBase, borderRight: '1px solid #000', color: '#555', whiteSpace: 'nowrap', padding: '3px 5px' }}>
                        <strong>{l2}</strong><br />
                        <div style={{ fontWeight: 'bold', fontSize: isMobile ? '9px' : '11.5px' }}>{v2}</div>
                      </td>
                    </tr>
                  ))}
                  <tr>
          ...
          <div style={{ marginBottom: 8, overflowX: isMobile ? 'visible' : 'auto' }}>
            <table style={{ 
              ...
              fontSize: isMobile ? '6.5px' : '10.5px',
            }}>
              <thead>
                <tr>
              ...
              <tbody>
                {safeProducts.length > 0 ? safeProducts.map((p, i) => (
                  ...
                  <tr>
                    <td colSpan={8} style={{ ...tdBase, textAlign: 'center', padding: '4px' }}>No products found</td>
                  </tr>
                )}
                ...
                {expenses.map((exp, i) => {
                  ...
                  return (
                    <tr key={`exp-${i}`}>
                      ...
                      <td style={{ ...tdBase, padding: '3px 5px' }} />
                      <td style={{ ...tdBase, padding: '3px 5px' }} />
                      <td style={{ ...tdBase, textAlign: 'right', color: val < 0 ? 'red' : 'black', padding: '3px 5px' }}>
                        {fmt(val)}
                      </td>
                    </tr>
                  )
          ...
          <div style={{ marginBottom: 8, wordBreak: 'break-word', fontSize: isMobile ? '8px' : '10.5px' }}>
                ...
                {Array.from(hsnMap.entries()).length > 0
                  ? Array.from(hsnMap.entries()).map(([hsn, rec], i) => (
                    <tr key={i}>
                      ...
                      <td style={{ ...tdBase, textAlign: 'right', padding: '3px 5px' }}>
          ...
          <div style={{
            ...
            <div style={{ width: '60%', wordBreak: 'break-word' }}>
              ...
              <p style={{ fontStyle: 'italic', marginTop: 3, marginBottom: 0, fontSize: isMobile ? '8px' : '10.5px' }}>
            ...
            <div style={{ textAlign: 'right', width: '40%' }}>
              ...
              <div style={{ marginTop: 30, fontSize: isMobile ? '8px' : '10.5px' }}>Authorised Signatory</div>
            </div>
          </div>

          <div style={{ textAlign: 'center', marginTop: 8, padding: 4, fontSize: isMobile ? '7px' : '9px' }}>
