...
const sendPriceListWhatsAppAlt = async (row) => {
    ...
    // Action Cell Component for WhatsApp button
    const ActionCell = ({ row, isPriceListTab = false }) => {
        const hasPhone = phoneMap.has(Number(row.Retailer_Id));
        const isLoading = sendingStates[row.Do_Id];
        
        return (
            <Box sx={{ display: 'flex', gap: 0.5 }}>
                <Tooltip title={hasPhone ? (isPriceListTab ? "Send Price List via WhatsApp" : "Send Invoice via WhatsApp") : "No phone number available"}>
                    <span>
                        <IconButton
                            size="small"
                            onClick={() => handleWhatsAppClick(row, isPriceListTab)}
    ...
    // Shared columns definition (used by both tabs)
    const baseColumns = [
        {
            Field_Name: "Select",
            isVisible: 1,
    ...
    // Sale Invoice Tab Columns (with DotPe WhatsApp)
    ...
    // Price List Tab Columns (with Askeva WhatsApp)
    ...
    return (
        <>
            ...
            {/* Sale Invoice Tab - Uses DotPe Service */}
            ...
            {/* Price List Tab - Uses Askeva Service */}
