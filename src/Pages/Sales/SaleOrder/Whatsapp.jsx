...
// Every candidate phone number available for a row, deduplicated, labelled by
// source (A1, Party Mobile 1, etc). Pure function (no React state) — used both
// by PhoneSelectCell to populate its dropdown and by resolveSendPhone (inside
// the Whatsapp component) as the default fallback chain when no manual
// override exists yet.
const getPhoneCandidates = (row, phoneMap) => {
    const list = [];
    const push = (label, v) => {
        if (!v) return;
    ...
    // A1 first — this is the default that pre-fills the field on initial load.
    push("A1", phoneMap?.get?.(Number(row.Retailer_Id)));
    push("A1", row.A1_Phone);
    push("A1", row.A1);
    push("Party Mobile 1", row.Party_Mobile_1);
    push("Party Mobile 2", row.Party_Mobile_2);
    push("Phone", row.Customer_Phone);
    push("Phone", row.retailerPhone);
    push("Phone", row.lolA1);
    push("Phone", row.phone);
    push("Phone", row.Mobile_No);
    return list;
};
...
const PhoneSelectCell = ({ row, tab, phoneMap, selectedPhones, setSelectedPhones }) => {
    ...
    const commit = (value) => {
        ...
        setLocalValue(norm);
        setSelectedPhones((prev) => ({ ...prev, [rowKey]: norm }));
    };

    const hasValid = isValidPhone(localValue);
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 0.25 }}>
            <TextField
                ...
                sx={{
                    ...
                    "& .MuiInputBase-input": { fontSize: 12, color: hasValid ? "inherit" : "#999" },
                }}
            />
            {candidates.length > 0 && (
                <>
                    ...
                    <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={() => setMenuAnchor(null)}>
                        {candidates.map((c, i) => (
                            <MenuItem
                                key={i}
                                selected={c.value === localValue}
                                onClick={() => { commit(c.value); setMenuAnchor(null); }}
                            >
                                <ListItemText primary={c.value} secondary={c.label} />
                            </MenuItem>
                        ))}
                    </Menu>
                </>
...
const sendViaAskeva = async ({ phone, templateName, language = "en", bodyParams, evatoken, imageUrl = null }) => {
    const components = [];
    
    // Add header with image if imageUrl is provided
    if (imageUrl) {
        components.push({
            type: "header",
            parameters: [
...
const SaleInvoiceLedgerFilterBar = ({ dataSource, columnFilters, setColumnFilters, fromDate, toDate, setFromDate, setToDate, onSearch, isLoading, showFilters = true }) => {
    const getValue = (row, key) => row?.ledgerLOLDetails?.[key];
    ...
    const handleClearAll = () => setColumnFilters({});
    ...
    return (
        <Box sx={{ p: 2, bgcolor: "background.paper", borderRadius: 1, mb: 2, border: "1px solid", borderColor: "divider" }}>
            <Stack direction="row" spacing={2} alignItems="center" flexWrap="wrap" mb={2}>
                ...
                />
                <Button
                    variant="contained" size="small" startIcon={<Search />}
                    onClick={onSearch}
            ...
            {showFilters && (
                <Stack direction="row" gap={2} flexWrap="wrap">
                    {LEDGER_LOL_FILTER_COLUMNS.map(({ key, label }) => {
                        const options = getUniqueValues(key);
                        const currentFilter = columnFilters[key];
                        return (
                            <Autocomplete
                                key={key} multiple size="small" options={options}
                                ...
                                renderInput={(params) => (
                                    <TextField
                                        {...params}
                                        label={label}
                                        placeholder={options.length === 0 ? "No data" : "Select to filter"}
                                        sx={{ minWidth: 220 }}
                                        InputProps={{
                                            ...params.InputProps,
                                            endAdornment: (
                                                <>
                                                    {options.length > 0 && (
                                                        <Chip label={`${options.length}`} size="small" color="info" sx={{ mr: 1, height: 18, fontSize: 9 }} />
                                                    )}
                                                    {params.InputProps.endAdornment}
                                                </>
                                            )
...
const SheetsheetFilterBar = ({ 
    ...
    const filterColumns = [
        { key: "Do_Inv_No", label: "Invoice No" },
        { key: "retailerNameGet", label: "Retailer Name" },
        { key: "retailerPhone", label: "Phone (A1)" },
        { key: "Delivery_Status", label: "Delivery Status" },
        { key: "imageStatus", label: "Image Status" },
        { key: "Created_BY_Name", label: "Created By" },
        { key: "LR_Uploaded_By_Name", label: "LR Uploaded By" },
        { key: "voucherTypeGet", label: "Voucher Type" },
    ];
...
const Whatsapp = ({ loadingOn, loadingOff, AddRights, EditRights, PrintRights, pageID }) => {
    ...
    const [previewDialog, setPreviewDialog] = useState({ open: false, row: null, tab: "", url: "" });
    ...
    // ---- Fetch guard refs (prevents flicker from duplicate / overlapping fetches) ----
    // tabFetchedRef: marks a tab's data as already loaded once, so clicking back into
    // it just reuses cached state instead of re-fetching and re-rendering.
    // tabFetchingRef: prevents two overlapping fetches for the same tab from firing
    // when a user clicks rapidly (2-3 times) before the first request resolves.
...
const sendWhatsAppMessage = useCallback(
    async ({ tab, phone, bodyParams, clientRefId, imageUrl = null }) => {
    ...
    const logWhatsappSend = async (row, tab) => {
        try {
            ...
            await fetchLink({
                address: "masters/whatsapp/logSend",
                method: "POST",
                bodyData: {
                    documentType: row.DocumentType,
                    referenceId: row.DocumentId,
                    retailerId: row.Retailer_Id,
                    retailerName: row.retailerNameGet || row.Retailer_Name,
                    mobileNo: phone,
                    messageTemplate: tab,
            ...
            setWhatsappCounts((prev) => {
                const key = getCountKey(row.DocumentType, row.DocumentId);
    ...
    // FETCH WHATSAPP COUNTS FOR A SPECIFIC DATA SET
    const fetchWhatsappCountsFor = async (rows, docTypeOverride = null) => {
        if (!rows || rows.length === 0) return;

        // Determine document type
        let docType = docTypeOverride;
        if (!docType) {
            docType = rows[0]?.DocumentType || rows[0]?.voucherTypeGet;
            if (!docType || docType === 'Unknown') {
                if (rows[0]?.Do_Id) docType = 'SalesInvoice';
                else if (rows[0]?.So_Id) docType = 'SalesOrder';
                else if (rows[0]?.Receipt_Id) docType = 'Receipt';
                else if (rows[0]?.Ret_Id) docType = 'PriceList';
                else if (rows[0]?.Acc_Id && rows[0]?.Bal_Amount !== undefined) docType = 'Outstanding';
            }
        }
        ...
        // Get unique IDs
        const ids = [...new Set(rows.map((r) => r.DocumentId || r.Receipt_Id || r.Ret_Id || r.So_Id || r.Do_Id || r.Id).filter(Boolean))];
        ...
        try {
            ...
            if (response?.success && response.data) {
                setWhatsappCounts((prev) => {
                    ...
                    response.data.forEach((item) => {
                        const key = getCountKey(docType, item.Reference_Id);
                        next[key] = item.Sent_Count;
                    });
                    return next;
                });
            ...
            console.error("Failed to fetch whatsapp counts:", e);
        }
        return null;
    };

    // FETCH ALL WHATSAPP COUNTS FOR ALL TABS
    const fetchAllWhatsappCounts = useCallback(async () => {
        const allData = {
            'SalesInvoice': allSalesInvoices,
            'SalesOrder': allSalesOrders,
            'PriceList': priceListRetailers,
            'Receipt': allReceipts,
            'Outstanding': allOutstanding,
            'PendingBill': allPendingBills,
            'shetsheet': sheetsheetData
        };

        const allCounts = {};
        let hasData = false;

        for (const [docType, rows] of Object.entries(allData)) {
            ...
            try {
                ...
                if (response?.success && response.data) {
                    response.data.forEach((item) => {
                        allCounts[getCountKey(docType, item.Reference_Id)] = item.Sent_Count;
                    });
                }
            } catch (e) {
                console.error(`Failed to fetch counts for ${docType}:`, e);
            }
        }

        if (hasData) {
            setWhatsappCounts(allCounts);
        }
        setCountsLoaded(true);
    }, [allSalesInvoices, allSalesOrders, priceListRetailers, allReceipts, allOutstanding, allPendingBills, sheetsheetData, getCountKey]);
    ...
    const processInvoice = (invoice, phoneMapRef) => ({
        ...
        A1_Phone: (phoneMapRef || phoneMap).get(Number(invoice.Retailer_Id)) || invoice.ledgerLOLDetails?.A1 || invoice.A1 || "Not Available",
    ...
    const fetchSaleInvoices = async (phoneMapRef = null) => {
        ...
        try {
            ...
            const response = await fetchLink({
                address: `sales/salesInvoice/lrReportWhatsapp?Fromdate=${saleInvoiceFromDate}&Todate=${saleInvoiceToDate}`,
                loadingOn, loadingOff,
            });
            if (response?.success && response.data) {
                const si = toArray(response.data).map((x) => processInvoice(x, phoneMapRef || phoneMap));
                setAllSalesInvoices(si);
                if (activeTab === "sale_invoice") {
                    setSalesInvoices(si);
                    setFilteredData(si);
                    await fetchWhatsappCountsFor(si, 'SalesInvoice');
                }
                setCostTypes(toArray(response?.others?.costTypes));
                setUniqueInvolvedCost(toArray(response?.others?.uniqeInvolvedStaffs));
                // Show the actual count returned for this date range, so it's always
                // clear whether a low number is coming from the server or from an
                // active filter (see onSearch above for the filter-clearing fix).
                toast.success(`Loaded ${si.length} sale invoice(s) for ${saleInvoiceFromDate} to ${saleInvoiceToDate}`);
            } else {
                setAllSalesInvoices([]);
                if (activeTab === "sale_invoice") setFilteredData([]);
                toast.info("No sale invoices found for the selected period");
            }
            tabFetchedRef.current.sale_invoice = true;
        } catch (e) {
            console.error(e);
            toast.error("Failed to load sale invoices");
        } finally {
            setIsSaleInvoiceLoading(false);
            tabFetchingRef.current.sale_invoice = false;
        }
    };
    ...
    // NOTE: Sale orders are already loaded once by the big init() effect below.
    // The duplicate `useEffect(() => { fetchSaleOrders(); }, [])` calls that used to
    // live here and further down (which caused sale orders to be fetched 2-3 times
    // on mount and produced the tab-click flicker) have been removed.

    const fetchSaleOrders = async (reqDate) => {
        if (tabFetchingRef.current.sale_order) return;
        tabFetchingRef.current.sale_order = true;
        setIsSaleOrderLoading(true);
        try {
            await fetchAllInvoices(true, reqDate);
        } finally {
            tabFetchingRef.current.sale_order = false;
        }
    };
    ...
    const fetchPendingInvoices = async () => {
        try {
            setIsLoading(true);
            setViewMode("pending");
            const soResp = await fetchLink({ address: `sales/salesOrder/pendingDetails?reqDate=${filters.reqDate}`, loadingOn, loadingOff });
            const so = toArray(soResp?.data).map((x) => processOrder(x, phoneMap));
    ...
    const fetchReceipts = async () => {
        if (tabFetchingRef.current.receipt_list) return;
        tabFetchingRef.current.receipt_list = true;
        try {
            setIsLoading(true);
            ...
            if (response?.success && response.data) {
                ...
                setAllReceipts(recs);
                await fetchWhatsappCountsFor(recs, 'Receipt');
                if (activeTab === "receipt_list") setFilteredData(recs);
                tabFetchedRef.current.receipt_list = true;
            } else {
                setAllReceipts([]);
            ...
            setIsLoading(false);
            tabFetchingRef.current.receipt_list = false;
        }
    };

    const fetchRetailersWithLOL = async () => {
        if (tabFetchingRef.current.price_list) return;
        tabFetchingRef.current.price_list = true;
        try {
            setIsLoading(true);
            ...
            if (response?.success && response.data) {
                ...
                await fetchWhatsappCountsFor(retailers, 'PriceList');
                tabFetchedRef.current.price_list = true;
            }
        } catch (e) { toast.error("Failed to load price list data"); }
        finally {
            setIsLoading(false);
            tabFetchingRef.current.price_list = false;
        }
    };
    ...
    const fetchSheetsheetData = async (phoneMapRef = null) => {
        ...
        try {
            ...
            if (response?.success && response.data) {
                ...
                setSheetsheetData([]);
                setFilteredSheetsheetData([]);
                if (activeTab === "shetsheet") setFilteredData([]);
                toast.info("No data found for the selected period");
            }
        } catch (error) {
            console.error("Error fetching shetsheet data:", error);
            toast.error("Failed to load shetsheet data");
        } finally {
            setIsSheetsheetLoading(false);
            tabFetchingRef.current.shetsheet = false;
        }
    };
    ...
    const fetchDebtorsCreditors = async (tab, fromDate, toDate) => {
        const guardKey = tab === "outstanding" ? "outstanding" : "pending_bills";
        if (tabFetchingRef.current[guardKey]) return;
        tabFetchingRef.current[guardKey] = true;
        try {
            setIsLoading(true);
            const response = await fetchLink({
            ...
            if (response?.success && response.data) {
                ...
                if (tab === "outstanding") {
                    ...
                    tabFetchedRef.current.outstanding = true;
                } else {
                    ...
                    tabFetchedRef.current.pending_bills = true;
                }
            ...
            setIsLoading(false);
            tabFetchingRef.current[guardKey] = false;
        }
    };
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const [soResp, retailersResp, receiptsResp, outstandingResp, pendingResp, sheetsheetResp] = await Promise.allSettled([
                ...
                fetchLink({ address: `receipt/receiptMasterwithLol?${new URLSearchParams(receiptFilters)}`, loadingOn, loadingOff }),
                fetchLink({ address: `reports/externalAPI/debtorsCreditors?Fromdate=${outstandingFromDate}&Todate=${outstandingToDate}`, loadingOn, loadingOff }),
                fetchLink({ address: `reports/externalAPI/debtorsCreditors?Fromdate=${pendingBillsFromDate}&Todate=${pendingBillsToDate}`, loadingOn, loadingOff }),
                fetchLink({ address: `sales/lrreportUpload?reqDate=${today}&Todate=${today}`, loadingOn, loadingOff })
            ]);

            if (soResp.status === 'fulfilled' && soResp.value?.success) {
                const so = toArray(soResp.value.data).map((x) => processOrder(x, phoneMapResult));
                setAllSalesOrders(so);
                if (activeTab === "sale_order") {
                    setSalesInvoices(so);
                    setFilteredData(so);
            ...
            if (retailersResp.status === 'fulfilled' && retailersResp.value?.success) {
                const retailers = toArray(retailersResp.value.data).map((r) => ({
                    ...r,
                    DocumentType: "PriceList",
                    DocumentId: r.Ret_Id,
                    DocumentNumber: `PL_${r.Ret_Id}`,
                    retailerNameGet: r.Retailer_Name,
                    A1_Phone: phoneMapResult.get(Number(r.Ret_Id)) || r.A1 || "Not Available",
                    Retailer_Id: r.Ret_Id,
                }));
                setPriceListRetailers(retailers);
                setFilteredPriceListRetailers(retailers);
            ...
            if (receiptsResp.status === 'fulfilled' && receiptsResp.value?.success) {
                const recs = toArray(receiptsResp.value.data).map((r) => ({
                    ...r,
                    DocumentType: "Receipt",
                    DocumentId: r.receipt_id || r.Receipt_Id || r.Id,
                    DocumentNumber: r.receipt_invoice_no || r.Receipt_No,
                    ...
                    createdOn: r.created_on || r.receipt_date,
                    Retailer_Id: r.Retailer_Id || r.Linked_Retailer_Id,
                    A1_Phone: phoneMapResult.get(Number(r.Retailer_Id || r.Linked_Retailer_Id)) || r.A1 || r.Customer_Phone || "Not Available",
                    Customer_Phone: r.A1 || r.Customer_Phone || "Not Available",
                    Alternate_Phone: r.A2 || r.Alternate_Phone || "",
                    Landline_Phone: r.A3 || r.Landline_Phone || "",
                    LOL_Id: r.LOL_Id || r.Id,
            ...
            if (outstandingResp.status === 'fulfilled' && outstandingResp.value?.success) {
                let records = toArray(outstandingResp.value.data).map((r) => ({
                    ...r,
                    DocumentType: "Outstanding",
                    DocumentId: r.Retailer_Id,
                    DocumentNumber: r.Acc_Id,
                    retailerNameGet: r.Retailer_Name,
                    A1_Phone: r["A1 - LOL-[19]"]?.trim() || phoneMapResult.get(Number(r.Retailer_Id)) || "Not Available",
            ...
            if (sheetsheetResp.status === 'fulfilled' && sheetsheetResp.value?.success) {
                ...
                setSheetsheetData(sheetsheetDataProcessed);
                setFilteredSheetsheetData(sheetsheetDataProcessed);
                if (activeTab === "shetsheet") setFilteredData(sheetsheetDataProcessed);
                tabFetchedRef.current.shetsheet = true;
            }
    ...
    const buildSheetsheetParams = (row) => {
        const companyname = companyInfo[0]?.Company_Name || "Company";
        const customerName = row.retailerNameGet || row.Retailer_Name || "Customer";
    ...
    const getSelectedRows = useCallback(() => {
        ...
        else if (activeTab === "shetsheet") source = filteredSheetsheetData;
        else source = filteredData;
        return source.filter((row) => ids.includes(toNumber(row.DocumentId)));
    }, [multipleCostCenterUpdateValues.Do_Id, activeTab, filteredPriceListRetailers, filteredData, filteredOutstanding, filteredPendingBills, filteredSheetsheetData]);
    ...
    const handleBulkSend = async (mode) => {
        ...
        const sendRow = async (row) => {
            try {
                let phone = resolveSendPhone(row, activeTab);
                if (!phone || !isValidPhone(phone)) {
                    increment(false);
                    return;
                }
    ...
    const bodyParams1 = [companyname, customerName,invoicedate,invoiceno]; 
                ...
                if (activeTab === "outstanding") {
                    ...
                    const params = buildSheetsheetParams(row);
                    tab = params.tab;
    ...
    // Single source of truth for "what number do we actually send to". Every send
    // path (single-row send, bulk send, outstanding/pending-bills send) calls this
    // instead of resolving phone fields itself, so a manual override made via the
    // Phone column dropdown/text entry (PhoneSelectCell, module-level component
    // above) is respected everywhere consistently.
    const resolveSendPhone = useCallback((row, tab) => {
        const rowKey = getRowKey(row, tab);
        const override = selectedPhones[rowKey];
        if (override && isValidPhone(override)) return override;
        const candidates = getPhoneCandidates(row, phoneMap);
        return candidates[0]?.value || "";
    }, [selectedPhones, phoneMap]);

    const ActionCell = ({ row, tab = "sale_invoice" }) => {
        const phone = resolveSendPhone(row, tab);
        ...
        const canPreview = PREVIEWABLE_TABS.includes(tab);
        ...
        return (
            <Box sx={{ display: "flex", gap: 0.5, alignItems: "center" }}>
                {canPreview && (
                    <Tooltip title={hasPhone ? "Preview before sending" : "No valid phone number available"}>
                        <span>
                            <IconButton size="small" onClick={() => openPreview(row, tab)} disabled={!hasPhone} color="primary">
                ...
                {sentCount > 0 && (
                    <Tooltip title={`Sent ${sentCount} time${sentCount > 1 ? 's' : ''}`}>
                        <Chip label={sentCount} size="small" color="success" sx={{ height: 18, minWidth: 18, fontSize: 10, fontWeight: 700 }} />
                    </Tooltip>
                )}
    ...
    const baseColumns = [
        ...
        },
        {
            ...
            Cell: ({ row }) => (
                <PhoneSelectCell row={row} tab={row.DocumentType === "SalesOrder" ? "sale_order" : "sale_invoice"} phoneMap={phoneMap} selectedPhones={selectedPhones} setSelectedPhones={setSelectedPhones} />
            ),
        },
        {
    ...
    const priceListColumns = [
        ...
        {
            Field_Name: "PhoneNumber", ColumnHeader: "Phone", isVisible: 1, isCustomCell: true,
            Cell: ({ row }) => <PhoneSelectCell row={row} tab="price_list" phoneMap={phoneMap} selectedPhones={selectedPhones} setSelectedPhones={setSelectedPhones} />
        },
    ...
    const receiptColumns = [
        ...
        {
            Field_Name: "PhoneNumber", ColumnHeader: "Phone", isVisible: 1, isCustomCell: true,
            Cell: ({ row }) => <PhoneSelectCell row={row} tab="receipt_list" phoneMap={phoneMap} selectedPhones={selectedPhones} setSelectedPhones={setSelectedPhones} />
        },
    ...
    const debtorsCreditorsColumns = [
        ...
        { Field_Name: "District", Fied_Data: "string", ColumnHeader: "District", isVisible: 1 },
        {
            ...
            Cell: ({ row }) => (
                <PhoneSelectCell row={row} tab={row.DocumentType === "Outstanding" ? "outstanding" : "pending_bills"} phoneMap={phoneMap} selectedPhones={selectedPhones} setSelectedPhones={setSelectedPhones} />
            )
        },
    ...
    // NOTE: Sale Invoice date/ledger filters (saleInvoiceFilters, from
    // SaleInvoiceLedgerFilterBar) and the WhatsApp column filters (columnFilters,
    // from WhatsAppFilterBar) both apply to the sale_invoice tab. They used to be
    // computed in two separate effects that each called setFilteredData
    // independently — a race where whichever effect ran last silently discarded
    // the other's filter. Both are now folded into applyFilters below so there is
    // a single pipeline: ledger filters first, then column filters, one
    // setFilteredData call.

    // Apply filters for sheetsheet tab
    useEffect(() => {
        if (activeTab === "shetsheet") {
            let filtered = [...sheetsheetData];
            for (const [key, values] of Object.entries(sheetsheetFilters)) {
                if (!values || values.length === 0) continue;
                filtered = filtered.filter((row) => {
                    const v = row[key];
                    if (v === undefined || v === null || v === "") return false;
                    return values.includes(String(v).trim());
                });
            }
            setFilteredSheetsheetData(filtered);
            setFilteredData(filtered);
        }
    }, [sheetsheetFilters, sheetsheetData, activeTab]);
    ...
    const applyFilters = useCallback(() => {
        ...
        if (activeTab === "sale_invoice") {
            for (const [key, values] of Object.entries(saleInvoiceFilters)) {
                if (!values || values.length === 0) continue;
                filtered = filtered.filter((row) => {
                    const v = row?.ledgerLOLDetails?.[key];
                    if (v === undefined || v === null || v === "") return false;
                    return values.includes(String(v).trim());
                });
            }
        ...
        for (const [colName, filterValues] of Object.entries(currentFilters)) {
            ...
            filtered = filtered.filter((item) => {
                ...
                if (itemValue === null || itemValue === undefined || itemValue === "") {
                    const mappings = {
                        ...
                        'PayGroup': ['Q_Pay_Group', 'PayGroup'],
                        'PayDays': ['Q_Pay_Days', 'PayDays'],
                        'Do_Inv_No': ['Do_Inv_No', 'DocumentNumber'],
                        'retailerPhone': ['retailerPhone', 'A1_Phone', 'A1', 'Phone'],
                        'Delivery_Status': ['Delivery_Status', 'Status'],
                        'imageStatus': ['imageStatus'],
                        'Created_BY_Name': ['Created_BY_Name', 'Created_By'],
                        'LR_Uploaded_By_Name': ['LR_Uploaded_By_Name', 'LR_Uploaded_By'],
                        'voucherTypeGet': ['voucherTypeGet', 'Voucher_Type'],
                ...
                return filterValues.some(filterVal => {
                    ...
                    if (colName === 'credit_amount' || colName === 'debit_amount' ||
                        ...
                        colName === 'totalBillQty' || colName === 'totalActQty') {
        ...
        for (const col of filterColumns) {
            const fv = currentFilters[col.Field_Name];
            if (!fv) continue;
            if (fv.type === "range") {
                const { min, max } = fv;
                filtered = filtered.filter((item) => {
    ...
    // ---- Guarded, cache-aware tab change handler ----
    // - Ignores a click on the tab that's already active.
    // - Ignores a click while that tab's data is still being fetched (stops
    //   rapid 2-3x clicking from firing overlapping requests / flicker).
    // - Fetches a tab's data only the FIRST time it's opened; every subsequent
    //   click just re-displays the cached state already in memory.
    const handleTabChange = (newVal) => {
        ...
        if (newVal === "price_list") {
            ...
            if (!tabFetchedRef.current.sale_invoice) {
                fetchSaleInvoices();
            } else {
            ...
            if (!tabFetchedRef.current.outstanding && allOutstanding.length > 0) {
                fetchWhatsappCountsFor(allOutstanding, 'Outstanding');
                tabFetchedRef.current.outstanding = true;
            }
        } else if (newVal === "pending_bills") {
            setFilteredData(allPendingBills);
            setFilteredPendingBills(allPendingBills);
            if (!tabFetchedRef.current.pending_bills && allPendingBills.length > 0) {
                fetchWhatsappCountsFor(allPendingBills, 'PendingBill');
                tabFetchedRef.current.pending_bills = true;
            }
        } else if (newVal === "shetsheet") {
            if (!tabFetchedRef.current.shetsheet) {
                if (sheetsheetData.length > 0) {
                    setFilteredSheetsheetData(sheetsheetData);
                    setFilteredData(sheetsheetData);
                    fetchWhatsappCountsFor(sheetsheetData, 'shetsheet');
                    tabFetchedRef.current.shetsheet = true;
                } else {
                    fetchSheetsheetData();
                }
    ...
    // NOTE: This mount effect used to fetch outstanding/pending bills a SECOND time
    // even though init() already fetches both. Removed to stop the duplicate
    // fetch → duplicate re-render flicker on the Transaction / Pending Bills tabs.
        ...
        <>
            <Box sx={{ borderBottom: 1, borderColor: "divider", mb: 2 }}>
                <Tabs value={activeTab} onChange={(_, v) => handleTabChange(v)} textColor="primary" indicatorColor="primary">
                    ...
                    <Tab label="shetsheet" value="shetsheet" />
                </Tabs>
            </Box>
            ...
            {activeTab === "sale_invoice" && (
                <>
                    <SaleInvoiceLedgerFilterBar
                        ...
                        fromDate={saleInvoiceFromDate}
                        toDate={saleInvoiceToDate}
                        setFromDate={setSaleInvoiceFromDate}
                        setToDate={setSaleInvoiceToDate}
                        onSearch={() => {
                            // A new date search should show exactly what that date range
                            // returns. Leftover Ledger/City/GSTIN/Phone selections (or
                            // WhatsApp column filters) from a PREVIOUS search were staying
                            // active and silently cutting down the row count for the new
                            // range without any visible warning — that's why a wider range
                            // could show fewer rows than a narrower one. Clearing both filter
                            // sets here means the count you see right after Search always
                            // matches the raw data fetched for the dates you picked.
