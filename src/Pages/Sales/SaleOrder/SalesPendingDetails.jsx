...
import { salesOrderDataSet, salesOrderDisplayColumns } from "./filters";
...
const PurchaseOrderDataEntry = ({ loadingOn, loadingOff, AddRights, EditRights, DeleteRights }) => {
    ...
    useEffect(() => {
        ...
        fetchLink({
            address: `sales/salesOrderPendingDelivery?Fromdate=${filters.Fromdate}&Todate=${filters.Todate}`,
        }).then(data => {
            if (data.success) {
    ...
    return (
        <>
            <FilterableTable
                ...
                tableMaxHeight={650}
                EnableSerialNumber
                title={'Sales Order - ' + filters.OrderStatus}
                maxHeightOption
                ExcelPrintOption
                PDFPrintOption
                ButtonArea={
