...
import { isEqualNumber, isGraterNumber, LocalDate, numberToWords, RoundNumber } from '../../../Components/functions';
...
const TaxInvoiceFull = ({ invoice, isDialog = false, loadingOn, loadingOff }) => {
    ...
    useEffect(() => {
        ...
        fetchLink({
            address: `sales/salesInvoiceById?Do_Id=${invoice}`,
            loadingOn,
            loadingOff
        }).then(data => {
            if (data.success && data.data && data.data.length > 0) {
                const invoiceData = data.data[0];
                setApiData(invoiceData);
                setOrderDetails(invoiceData);
                setOrderProducts(invoiceData?.Products_List || []);
    ...
    useEffect(() => {
        ...
        fetchLink({
            address: `masters/retailers/info?Retailer_Id=${currentInvoice?.Retailer_Id}`
        })
        .then(data => {
            if (data?.success) {
    ...
    }, [currentInvoice?.Retailer_Id]);

    const handlePrint = useReactToPrint({
        content: () => printRef.current,
        documentTitle: `Invoice-${currentInvoice?.Do_Inv_No || 'Invoice'}`,
        pageStyle: `
            @page {
                size: A4 portrait;
                margin: 10mm;
    ...
    useEffect(() => {
        if (useProcessedData && orderDetails && orderProducts && orderProducts.length > 0 && 
            Object.keys(retailerInfo).length > 0 && Object.keys(companyInfo).length > 0) {
            ...
            const processedInv = {
                ...
                Do_Inv_No: orderDetails?.Do_Inv_No,
                Do_Date: LocalDate(orderDetails?.Do_Date),
                ...
                Buyer_Order_No: orderDetails?.Buyer_Order_No || '',
                Buyer_Order_Date: orderDetails?.Buyer_Order_Date || '',
                Dispatch_Doc_No: orderDetails?.Dispatch_Doc_No || '',
                Delivery_Note_Date: orderDetails?.Delivery_Note_Date || '',
                LR_No: orderDetails?.LR_No || '',
    ...
    const safeProducts = useProcessedData ? processedProducts : 
    ...
    const broker = Staffs_Arrays.find(
        staff => staff?.Involved_Emp_Type === "Broker" || staff?.Emp_Type_Id === 5
    );
    ...
    safeProducts.forEach((p) => {
        ...
        rec.taxable += Number(p.Taxable_Amount) || 0;
        rec.cgstAmt += Number(p.Cgst_Amo) || 0;
        rec.sgstAmt += Number(p.Sgst_Amo) || 0;
        rec.igstAmt += Number(p.Igst_Amo) || 0;
    });
    ...
    if (!safeInvoice || Object.keys(safeInvoice).length === 0) {
        return <div style={{ padding: '20px', border: '2px solid #ccc' }}>No invoice data available</div>;
    }
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '1100px', margin: isDialog ? '0 auto' : '20px auto' }}>
            ...
            >
                {companyInfo?.Company_id == 4 ? (
                    <h2 style={{ textAlign: "center", margin: 0 }}>TAX INVOICE</h2>
                ) : (
                    <h2 style={{ textAlign: "center", margin: 0 }}>Sales Invoice</h2>
                )} 
                ...
                <div style={{ ...borderStyle, marginBottom: '15px' }}>
                    <div style={{ display: 'flex' }}>
                        ...
                        <div style={{ width: '50%', ...borderRight, padding: '10px' }}>
                            ...
                            <div style={{ ...borderBottom, ...borderTop, paddingBottom: '10px', marginBottom: '8px' }}>
                                ...
                                {safeInvoice.shippingName || retailerInfo.Retailer_Name}
                                <br />
                                {safeInvoice.shippingDeliveryAddress || retailerInfo.Reatailer_Address} <br/>
                                Phone No: {safeInvoice.Mobile_No || retailerInfo.Mobile_No} <br/>
                                GSTIN/UIN : {safeInvoice.shippingGstNumber || retailerInfo.Gstno}<br />
                                State Name : {safeInvoice.shippingStateName || retailerInfo.StateGet}
                            </div>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                            <thead>
                                <tr style={borderBottom}>
                                    ...
                                    <th style={{ padding: '8px', backgroundColor: '#f0f0f0' }}>Amount</th>
                                </tr>
                            </thead>
                            <tbody>
                                {safeProducts.length > 0 ? (
                                    safeProducts.map((p, idx) => (
                                        <tr key={idx}>
                                           ...
                                           <td style={{ padding: '8px', ...borderRight }}>
                                            ...
                                            <td style={{ padding: '8px', ...borderRight, textAlign: 'center' }}>{p.Bag || 0}</td>
                                    ...
                                    <tr>
                                        <td style={{ padding: '8px', textAlign: 'center' }} colSpan="9">No products found</td>
                                    </tr>
                                )}
                                ...
                                {expenseArray.length > 0 &&
                                    expenseArray.map((exp, index) => {
                                        const netAmount = Number(exp?.Expence_Value) || 0;
                                        return (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', ...borderStyle, padding: '15px', marginTop: '15px' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                            <div>for {companyInfo.Company_Name}</div>
