...
const ParameterDialog = ({ open, onClose, itemData, moduleParameters = [], itemParameters = {}, setItemParameters }) => {
    const [selectedParameters, setSelectedParameters] = useState([]);
    const [parameterValues, setParameterValues] = useState({});

    const itemId = itemData?.Item_Id;
    const itemName = itemData?.ProductName || 'Item';

    useEffect(() => {
        if (open && itemId) {
            const existingParams = toArray(itemParameters[itemId]);
            const initialSelected = [];
            const initialValues = {};

            existingParams.forEach(param => {
                const matched = moduleParameters.find(mp => isEqualNumber(mp.numID, param.ParameterId));
                if (matched) {
                    initialSelected.push({
                        value: matched.numID,
                        label: matched.parameterName,
                        dataType: matched.dataType
                    });
                    initialValues[matched.numID] = {
    ...
    }, [open, itemId, itemParameters, moduleParameters]);
    ...
    const handleSave = () => {
        const formatted = selectedParameters.map(param => ({
            ItemId: itemId,
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            <DialogTitle>Parameters - {itemName}</DialogTitle>
            <DialogContent>
            ...
            <DialogActions>
                ...
                <Button onClick={handleSave} variant="contained" color="primary">Save Parameters</Button>
            </DialogActions>
        </Dialog>
    );
};
