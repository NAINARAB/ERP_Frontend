...
const ConvertScreen = ({ loadingOn, loadingOff }) => {
    ...
    // New state for common account feature
    const [applyCommonAccount, setApplyCommonAccount] = useState(false);
    const [commonAccountDetails, setCommonAccountDetails] = useState({
        selectedGroup: '',
        selectedGroupLabel: '',
        selectedAccount: '',
        selectedAccountName: '',
    ...
    const handleCommonGroupSelect = (groupId, groupLabel) => {
        const filtered = filterAccountsByGroupIds(groupId, baseData.accountGroupData, baseData.accountsList);
        
        setCommonAccountDetails(prev => ({
            ...prev,
    ...
    const handleCommonAccountSelect = (accountId, accountName) => {
        setCommonAccountDetails(prev => ({
            ...prev,
        ...
        if (applyCommonAccount && accountId) {
            applyCommonAccountToAllRows(accountId, accountName);
        }
    };

 
    const applyCommonAccountToAllRows = (accountId, accountName) => {
        setRowConfigs(prevConfigs => 
            prevConfigs.map(config => {
                if (transactionType === 'payment') {
                    return {
                        ...config,
                        debit_ledger: String(accountId),
                        debit_ledger_name: accountName
                    };
                } else {
                    return {
                        ...config,
                        credit_ledger: String(accountId),
    ...
    const handleApplyCommonAccountChange = (event) => {
        const checked = event.target.checked;
        setApplyCommonAccount(checked);
        
        if (checked && commonAccountDetails.selectedAccount) {
            applyCommonAccountToAllRows(
                commonAccountDetails.selectedAccount, 
                commonAccountDetails.selectedAccountName
            );
        }
    };
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
            {rowConfigs.map((config, idx) => {
                ...
                const accountOptions = (transactionType === 'payment' 
                    ? (config.selectedDebitGroup ? config.filteredDebitAccounts : baseData.accountsList)
                    : (config.selectedCreditGroup ? config.filteredCreditAccounts : baseData.accountsList)
                ).map(account => ({
                ...
                return (
                    <tr key={config.id} style={{ borderBottom: '1px solid #ddd' }}>
                        ...
                        <td style={{ padding: '12px', textAlign: 'left', minWidth: '250px', maxWidth: '280px' }}>
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                                    <Box>
                                        ...
                                        <Autocomplete
                                            ...
                                            value={accountOptions.find(opt => opt.id === config.debit_ledger) || null}
                                            onChange={(event, newValue) => {
                                                if (newValue) {
                                                    ...
                                                    handleRowFieldChange(config.id, 'debit_ledger', '');
                                                    handleRowFieldChange(config.id, 'debit_ledger_name', '');
                                                }
                                            }}
                                            renderInput={(params) => (
                                                <TextField
                                                    {...params}
                                                    size="small"
                                                    placeholder="Search account by typing..."
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.8 }}>
                                    <Box>
                                        ...
                                        <Autocomplete
                                            options={accountOptions}
                                            getOptionLabel={(option) => option.name || ''}
                                            value={accountOptions.find(opt => opt.id === config.credit_ledger) || null}
                                            onChange={(event, newValue) => {
                                                if (newValue) {
                                                    handleRowFieldChange(config.id, 'credit_ledger', newValue.id);
                                                    handleRowFieldChange(config.id, 'credit_ledger_name', newValue.name);
                                                } else {
                                                    handleRowFieldChange(config.id, 'credit_ledger', '');
                                                    handleRowFieldChange(config.id, 'credit_ledger_name', '');
                                                }
                                            }}
                                            renderInput={(params) => (
                                                <TextField
                        ...
                        <td style={{ padding: '12px', textAlign: 'center' }}>
                            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
                                ...
                                {config.selectedInvoices && config.selectedInvoices.length > 0 ? (
                                    <Chip 
                                        label={`${config.selectedInvoices.length}`}
                                        color="primary"
                                        size="small"
                                        variant="filled"
                                    />
                                ) : (
                                    <span style={{ color: '#999', fontSize: '12px' }}>0</span>
                                )}
                            </Box>
                        </td>
                    </tr>
                    ...
                    <Grid container spacing={2} alignItems="center">
                        <Grid item xs={12} md={3}>
                            <FormControlLabel
                                control={
                                    <Checkbox
                                        checked={applyCommonAccount}
                                        onChange={handleApplyCommonAccountChange}
                                        color="primary"
                                    />
                                }
                                label={
                                    <Typography variant="body2" fontWeight="bold">
                                        Apply Common {transactionType === 'payment' ? 'Debit' : 'Credit'} Account
                                    </Typography>
                                }
                            />
                        </Grid>
                        ...
                        {applyCommonAccount && (
                            <>
                                <Grid item xs={12} md={4}>
                                    <Typography variant="caption" gutterBottom>
                                        {transactionType === 'payment' ? 'Debit' : 'Credit'} Account <span style={{ color: 'red' }}>*</span>
                                    </Typography>
                                    <Autocomplete
                                        options={commonAccountDetails.selectedGroup 
                                            ? commonAccountDetails.filteredAccounts 
                                            : baseData.accountsList
                                        }
                                        getOptionLabel={(option) => option.Account_name || ''}
                                        value={baseData.accountsList.find(acc => String(acc.Acc_Id) === commonAccountDetails.selectedAccount) || null}
                                        onChange={(event, newValue) => {
                                            if (newValue) {
                                                handleCommonAccountSelect(
                                                    String(newValue.Acc_Id), 
                                        ...
                                        isOptionEqualToValue={(option, value) => String(option.Acc_Id) === String(value?.Acc_Id)}
                                        noOptionsText="No accounts found"
                                        fullWidth
                                    />
                                </Grid>
                                ...
                                {commonAccountDetails.selectedAccountName && (
                                    <Grid item xs={12} md={3}>
                                        <Chip
                                            label={`Selected: ${commonAccountDetails.selectedAccountName}`}
                                            color="success"
                                            variant="outlined"
                                            sx={{ fontWeight: 500 }}
                                        />
