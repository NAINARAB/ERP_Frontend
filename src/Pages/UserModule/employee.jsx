...
import { useEffect, useState } from 'react';
import { ChevronLeft, Add, KeyboardArrowUp, KeyboardArrowDown, Edit, Delete } from '@mui/icons-material';
import { toast } from 'react-toastify';
import { Card, CardContent, Collapse, IconButton } from '@mui/material';
import { ISOString, LocalDate } from '../../Components/functions';
import { fetchLink } from '../../Components/fetchComponent';
...
const EmployeeMaster = () => {
    ...
    const initialEmpValue = {
        ...
        user_manage_id: '',
        enter_by: parseInt(storage?.UserId),
        Company_id: parseInt(storage?.Company_id) || 4,
        fingerPrintEmpId: null,
        Department_ID: '',
        department: '',
        location: ''
    ...
    useEffect(() => {
        ...
        fetchLink({
            ...
            if (data.success) {
                setEmpData(data.data.sort((a, b) => Number(b.Emp_Id) - Number(a.Emp_Id)))
            }
        }).catch(e => console.log(e));
    ...
    const deleteEmp = (empId) => {
        if (window.confirm('Are you sure you want to delete this employee?')) {
            fetchLink({
                address: `userModule/employee`,
                method: 'DELETE',
                bodyData: { ID: empId, Company_id: parseInt(storage?.Company_id) || 4 }
            }).then(data => {
                if (data.success) {
                    toast.success(data.message);
                    setRefresh(!refresh);
    ...
    const setVal = (emp, method) => {
        setPK(emp.Emp_Id);
        if (method === 'PUT') {
            setUserCreate(emp?.User_Mgt_Id ? true : false);
            setEmpFormData(pre => ({
                ...pre,
                branch: emp?.Branch,
                empname: emp?.Emp_Name,
                        ...
                        <div style={{ maxHeight: '74vh', overflowY: 'scroll' }} className='p-3 pe-2'>
                            ...
                            <div className="table-responsive">
                                <table className="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        {(filteredData && filteredData.length ? filteredData : search === '' ? empData : []).map((emp, index) => (
                                            <tr key={emp.Emp_Id}>
                                                <td>{index + 1}</td>
                                                <td>{emp.Emp_Name || '-'}</td>
                                                ...
                                                <td>{emp.Mobile_No || '-'}</td>
                                                <td>
                                                    <IconButton size='small' onClick={() => setVal(emp, 'PUT')}><Edit /></IconButton>
                                                    <IconButton size='small' onClick={() => deleteEmp(emp.Emp_Id)}><Delete sx={{ color: '#FF6865' }} /></IconButton>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
