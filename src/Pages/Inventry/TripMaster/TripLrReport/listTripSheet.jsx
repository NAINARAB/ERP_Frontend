...
import { checkIsNumber, isEqualNumber, ISOString, toArray, toNumber, LocalDateWithTime } from "../../../../Components/functions";
...
const TripListLRReport = ({ loadingOn, loadingOff, AddRights, EditRights }) => {
    ...
    const columns = useMemo(
        () => [
            { Field_Name: "PR_Inv_Id", Fied_Data: "string", ColumnHeader: "Trip ID" },
    ...
    const fetchAllTrips = async (refresh = false) => {
        try {
            if (!refresh) {
                setIsLoading(true);
            } else {
            ...
            const data = await fetchLink({
                address: `inventory/tripSheet/lrReport?reqDate=${filters.reqDate}&staffStatus=${filters.staffStatus}`,
                loadingOn,
                loadingOff,
            });

            if(data.success) {
                setTripList(toArray(data.data));
            ...
            toast.error("Failed to load trip records");
        } finally {
            setIsLoading(false);
    ...
    }, [filters.reqDate, hasInitialLoading]);
    ...
    const onChangeEmployee = (tripRow, selectedOptions, costType) => {
        setFilters((prev) => {
            const updatedInvolvedStaffs = toArray(prev.selectedTrip?.involvedStaffs)
    ...
    const postAssignCostCenters = async (e) => {
        e.preventDefault();
        fetchLink({
            address: "inventory/tripSheet/lrReport",
            method: "POST",
            bodyData: {
                PR_Id: filters.selectedTrip?.PR_Id,
                involvedStaffs: filters.selectedTrip?.involvedStaffs,
                staffInvolvedStatus: toNumber(filters.selectedTrip?.staffInvolvedStatus),
            },
            loadingOn,
            loadingOff,
        })
    ...
    const postMultipleCostCenterUpdate = async () => {
        fetchLink({
            address: "inventory/tripSheet/lrReport/multiple",
            method: "POST",
            bodyData: {
    ...
    const postMultipleStaffRemove = async () => {
        if (!multipleStaffRemoveValues.CostCategory.value || multipleStaffRemoveValues.PR_Id.length === 0) {
            toast.error("Please select a Cost Category and at least one trip record");
            return;
        }

        fetchLink({
            address: "inventory/tripSheet/lrReport/multipleDelete",
            method: "POST",
            bodyData: {
    ...
    useEffect(() => {
        applyFilters();
    }, [columnFilters, tripList, filterColumns]);
        ...
        if (Fied_Data === "string") {
            const rawValues =
                typeof column.getFilterValues === "function"
                    ? tripList.flatMap((item) => column.getFilterValues(item) || [])
                    : tripList.map((item) => item[Field_Name]);
        ...
        <>
            <FilterableTable
                ...
                columns={[
                    ...
                    createCol("PR_Inv_Id", "string", "Trip ID"),
                    {
                        Field_Name: "",
                        isVisible: 1,
                        isCustomCell: true,
                ...
                ButtonArea={
                    <>
                        ...
                        <IconButton
                            size="small"
                            onClick={() => {
                                const selectedTripRows = tripList.filter(inv =>
                                    multipleCostCenterUpdateValues.PR_Id.includes(toNumber(inv.PR_Id))
                                );
                                
                                const allHaveStaffInvolved = selectedTripRows.length > 0 && 
                                    selectedTripRows.every(inv => isEqualNumber(inv.staffInvolvedStatus, 1));
                        ...
                        >
                            <option value="1">ALL TRIPS</option>
                            <option value="0">INCOMPLETED TRIPS</option>
                        </select>
                    </>
                }
            />
            ...
            <Dialog open={filters.assignDialog} onClose={onCloseAssignDialog} maxWidth="lg" fullWidth>
                <DialogTitle>
                    Assign Cost Centers: <span className="text-primary">{filters.selectedTrip?.PR_Inv_Id}</span>
                </DialogTitle>
                    ...
                    <DialogContent>
                        <div className="row">
                            {costTypes
                                ...
                                .map((costType, index) => {
                                    const tripEmployee = toArray(filters.selectedTrip?.involvedStaffs);
                                    const empCostType = tripEmployee
                                        .filter((emp) => isEqualNumber(emp.Emp_Type_Id, costType.Cost_Category_Id))
                                        ...
                                        <div className="col-lg-4 col-md-6 p-2" key={index}>
                                            ...
                                            <Select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    onChangeEmployee(filters.selectedTrip, values, costType);
                                                }}
                            ...
                            <div className="col-lg-4 col-md-6 p-2 d-flex align-items-end">
                                <input
                                    ...
                                    type="checkbox"
                                    id="removeFromList"
                                    checked={filters.selectedTrip?.staffInvolvedStatus === 1}
                                    onChange={(e) => {
...
export default TripListLRReport;
