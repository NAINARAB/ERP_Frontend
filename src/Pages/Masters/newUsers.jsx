...
const Users = ({ loadingOn, loadingOff }) => {
    ...
    // ---- saveUser ----
    // CREATE: encrypt the freshly typed password.
    // UPDATE: Password field is never shown/edited on this screen, so
    // inputValue.Password still holds the ORIGINAL encrypted value
    // (set untouched in editRow) -> send it back as-is. This satisfies
    // backend "required and valid" checks while never corrupting it.
    const saveUser = () => {
        const passwordToSend = inputValue.UserId
            ? inputValue.Password
            : encryptPasswordFun(inputValue.Password);

        fetchLink({
            address: `masters/users`,
            method: inputValue.UserId ? "PUT" : "POST",
            bodyData: { ...inputValue, Password: passwordToSend },
        }).then((data) => {
    ...
    // ---- editRow ----
    // Store the RAW encrypted Password from the row as-is. Do NOT decrypt it.
    // This is what gets sent back unchanged on save since the update
    // screen has no password field at all.
    const editRow = (user) => {
        const { UserId, Name, UserName, UserTypeId, Password, BranchId } = user;
                            ...
                            <form onSubmit={e => {
                                ...
                                <div className="row p-3">
                                    ...
                                    {/* Password field only shown when creating a new user.
                                        On update, nothing is rendered here at all. */}
                                    {!inputValue.UserId && (
                                        <div className="col-lg-4 col-md-6 p-2">
                                            <label>Password</label>
                                            <input
                            ...
                            <table className="table m-0">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <Td>
                                            <select
                                                value={inputValuecost.UserTypeId || ''}
                                                onChange={e => setInputValuecost(prev => ({ ...prev, UserTypeId: e.target.value }))}
                                                className="cus-inpt p-2"
                                                required
                                                aria-label="Select User Type"
