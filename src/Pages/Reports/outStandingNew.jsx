...
function OutStandingNew({ loadingOn, loadingOff }) {
    ...
    // Fetch distinct File_No for dropdown
    const fetchFileNoOptions = async () => {
        try {
            const res = await fetchLink({
                address: `receipt/outStandingAbove?reqDate=${encodeURIComponent(getTodayDate())}`,
            });
            const data = Array.isArray(res) ? res : res?.data || [];
            const uniqueFileNos = [...new Set(data.map((item) => item.File_No).filter(Boolean))];
    ...
    // Apply filter when File_No changes
    useEffect(() => {
        const sourceData =
            activeButton === "tillBilling" ? originalTillBilling : originalNoBilling;

        if (!filters.File_No) {
            setSalesReceipts(sourceData);
            setTotal_Invoice_value(calculateTotal(sourceData));
        } else {
            const filtered = sourceData.filter((row) => row.File_No === filters.File_No);
            setSalesReceipts(filtered);
            setTotal_Invoice_value(calculateTotal(filtered));
        }
    }, [filters.File_No, activeButton, originalTillBilling, originalNoBilling]);
                ...
                ButtonArea={
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        ...
                        <Select
                            ...
                            onChange={(e) =>
                                setFilters({ ...filters, File_No: e.target.value })
                            }
                            displayEmpty
                            sx={{ minWidth: 150, height: 40 }}
                        >
                        ...
                        {/* Search button (for date refresh) */}
                        <Tooltip title="Search">
                            <IconButton
                                color="primary"
                                size="medium"
