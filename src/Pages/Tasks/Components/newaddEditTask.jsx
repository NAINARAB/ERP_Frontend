...
const TaskMasterMgt = ({ row, children, openAction, reload, onCloseFun, loadingOn, loadingOff, onTaskAdded, Task_Type_Id, Task_Type }) => {
    ...
    useEffect(() => {
        if (isValidObject(row)) {
            ...
            setInputValue(prev => ({
                ...prev,
                ...row,
                Task_Group_Id: Task_Type_Id || row.Task_Group_Id || "",
            }));
            setIsEdit(true);
        } else {
            setInputValue(prev => ({
                ...prev,
                Task_Group_Id: Task_Type_Id || "",
            }));
            setIsEdit(false);
        }
    }, [row, Task_Type_Id]);
    ...
    const closeDialog = () => {
        ...
        setInputValue(prev => ({
            ...initialValue,
            Task_Group_Id: Task_Type_Id || "", // Preserve Task_Type_Id when closing
        }));
        if (onCloseFun) onCloseFun();
    };
    ...
    const postAndPutTask = async () => {
        // Ensure Task_Group_Id is properly set
        const finalTaskGroupId = Task_Type_Id || inputValue.Task_Group_Id;
        ...
        const PostObj = {
            ...inputValue,
            Task_Group_Id: finalTaskGroupId, // Use the properly set Task_Group_Id
            Task_Parameters: paramArr,
        };

        console.log("Submitting with Task_Group_Id:", finalTaskGroupId); // Debug log
        ...
        try {
            ...
            if (response.success) {
                ...
                setInputValue(prev => ({
                    ...initialValue,
                    Task_Group_Id: Task_Type_Id || "", // Reset with preserved Task_Type_Id
                }));
                closeDialog();
                if (reload) reload();
                if (onTaskAdded) onTaskAdded(); 
                    ...
                    <DialogContent>
                        <div className="row">
                                ...
                                {Task_Type_Id ? (
                                    <div>
                                        <input 
                                            type="text"
                                            className="cus-inpt"
                                            value={Task_Type || taskGroup.find(tg => tg.Task_Type_Id === Task_Type_Id)?.Task_Type || ""}
                                            readOnly
                                        />
                            ...
                            <div className="col-md-4 p-2">
                                <label>Base Task</label>
                                <select
                                    value={inputValue.Under_Task_Id || 0}
                                    className="cus-inpt"
                                    onChange={e => setInputValue({ ...inputValue, Under_Task_Id: e.target.value })}>
                                    <option value={0}>Primary</option>
                                    {taskData.map((o, i) => (
